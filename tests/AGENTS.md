...
1. Unit Tests: these are NUnit test methods that completely run in memory when executed, that is, they do not make I/O calls to third-party systems like databases, web services, message brokers, etc. There is one exception to this rule: Unit Tests are allowed to read from and write to the local file system as it is always present on the executing dev machine or within CI job runners. For Takt, the following in-process setups are therefore still Unit Tests:
   - Storage tests that run LiteDB against a temporary database file. LiteDB is an embedded library and the file system exception applies, so the whole storage layer is tested with the real production code instead of test doubles.
   - `JiraCloudClient` tests that run against a stubbed `HttpMessageHandler`. The HTTP pipeline executes entirely in-process and no socket is opened.
   - Headless UI tests using `Avalonia.Headless.NUnit`. The UI runs in-process without a real windowing system; this is the Avalonia analog of ASP.NET Core's Test Server.
2. Integration tests: these are test methods where I/O calls to third-party systems (as mentioned above) are performed. The defining factor is the existence of at least one I/O call that does not involve the local file system. For Takt, the only third-party system is Jira Cloud, which cannot be provided locally via Testcontainers, so an integration test means a real HTTP call to a Jira Cloud instance. Such tests require credentials and mutate real issues: keep them few, target a dedicated sandbox issue, and skip them (via `Assert.Ignore`) when the `TAKT_JIRA_TEST_*` environment variables are not present, so the default test run stays green without any setup.
3. An End-to-End test is a modification of an integration test where test doubles are not allowed. Everything has to be in place as it would run in a staging or production environment. For Takt — a desktop app plus Jira Cloud — automated End-to-End tests are deliberately out of scope: verifying the full chain requires a real desktop session and real worklogs, which is covered by the manual smoke checklists in the milestone plans (on Windows and Linux). Do not build automated End-to-End infrastructure for this repository.
